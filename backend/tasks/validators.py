...
def validate_attachment_file(uploaded_file):
    ...
    if uploaded_file.size > max_size:
        raise ValidationError(
            f"File size exceeds the maximum limit of 10MB (current size: {uploaded_file.size / (1024*1024):.2f}MB)."
        )
    ...
    if not ext or ext not in allowed_extensions:
        raise ValidationError(
            f"File extension '.{ext}' is not allowed. Supported formats: {', '.join(sorted(allowed_extensions))}"
        )
